...
fun EditarPropiedadScreen(
    ...
    val propiedadInicial = remember { propiedadAEditar }

    var titulo by remember { mutableStateOf(propiedadInicial.titulo) }
    var descripcion by remember { mutableStateOf(propiedadInicial.descripcion ?: "") }
    var tipoInmueble by remember { mutableStateOf(propiedadInicial.tipo_inmueble) }
    var estadoNegocio by remember { mutableStateOf(propiedadInicial.estado_negocio) }
    var precio by remember { mutableStateOf(propiedadInicial.precio) }
    var direccion by remember { mutableStateOf(propiedadInicial.direccion) }
    var habitaciones by remember { mutableStateOf(propiedadInicial.habitaciones.toString()) }
    var banos by remember { mutableStateOf(propiedadInicial.banos.toString()) }
    var areaMetros by remember { mutableStateOf(propiedadInicial.area_metros) }
    ...
    Scaffold(topBar = { TopAppBar(title = { Text("Modificar Propiedad") }) }) { innerPadding ->
        Column(
            ...
            Card(modifier = Modifier.fillMaxWidth(), colors = CardDefaults.cardColors(containerColor = MaterialTheme.colorScheme.surfaceVariant)) {
                Column(modifier = Modifier.padding(16.dp)) {
                    ...
                    Text(text = if (propiedadInicial.imagen != null) "Tiene foto cargada en el servidor" else "No tiene foto previa", style = MaterialTheme.typography.bodySmall)
                    Spacer(modifier = Modifier.height(8.dp))
                    Button(onClick = { galeriaLauncher.launch("image/*") }) {
                        Text(if (imagenUri != null) "🔄 Reemplazar Foto Elegida" else "🖼️ Subir Nueva Foto")
                    }
                    if (imagenUri != null) {
                        Text(text = "Nueva foto lista para actualizar", style = MaterialTheme.typography.bodySmall, color = MaterialTheme.colorScheme.primary, modifier = Modifier.padding(top = 4.dp))
                    }
                }
            }
            ...
            Button(
                onClick = {
                    ...
                    val payload = Propiedad(
                        id = propiedadInicial.id, titulo = titulo, descripcion = descripcion.ifEmpty { null },
                        ...
                        banos = banos.toIntOrNull() ?: 0, area_metros = areaMetros,
                        agente = aId, zona = zId, imagen = propiedadInicial.imagen
                    )

                    viewModel.guardarPropiedad(token, payload, partImagen, esEdicion = true) { exitoso ->
                        cargando = false
                        if (exitoso) onBackClick() else mensajeError = "Error al actualizar la propiedad."
                    }
                },
