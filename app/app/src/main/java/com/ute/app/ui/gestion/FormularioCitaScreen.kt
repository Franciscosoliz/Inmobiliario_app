...
fun CrearCitaScreen(
    ...
    var estadoSeleccionado by remember { mutableStateOf("Programada") }
    var comentarios by remember { mutableStateOf("") }
    var propiedadSeleccionadaId by remember { mutableStateOf<Int?>(null) }
    ...
    Scaffold(
        topBar = { TopAppBar(title = { Text("Nueva Cita") }) }
    ) { innerPadding ->
        Column(
            modifier = Modifier
                .fillMaxSize()
            ...
            Button(
                onClick = {
                    ...
                    cargando = true
                    val nuevaCita = Cita(id = 0, propiedad = pId, cliente = cId, agente = aId, fecha_hora = fechaHoraIso, comentarios = comentarios, estado = estadoSeleccionado)

                    viewModel.guardarCita(token, nuevaCita, esEdicion = false) { exitoso ->
                        cargando = false
